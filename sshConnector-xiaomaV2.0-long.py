...
class SSHClientApp:
    def __init__(self, root):
        self.root = root
        self.root.title("SSH 客户端 小马版v2.0  一小时版")
        ...
        self.reconnect_delay = 10  # 重连等待时间(秒)
    ...
    def execute_commands(self, commands):
        try:
                    ...
                    if transport and transport.is_active():
                        ...
                        while not self.stop_event.is_set() and self.microcom_active:
                            try:
                                ...
                                self.output(f"读取错误: {str(e)}")
                                self.handle_connection_loss()
                                break
                except (paramiko.SSHException, OSError) as e:
                    self.output(f"创建microcom会话失败: {str(e)}")
                    self.handle_connection_loss()
            ...
            if self.connected:
                self.export_btn.config(state="normal")
                self.output(f"命令执行完成，已获得 {len(self.signal_strengths)} 条数据，可以导出结果")
            ...
            if '+QENG: "servingcell"' in line:
                ...
                if len(parts) >= 15:
                    try:
                        ...
                        self.signal_time.append(datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
    ...
    def handle_connection_loss(self):
        ...
        self.status_var.set("状态: 连接中断，尝试重连")
        ...
        for attempt in range(1, self.max_reconnect_attempts + 1):
            ...
            try:
                self.output(f"重连尝试 {attempt}/{self.max_reconnect_attempts}...")
                host = self.host_entry.get()
                port = int(self.port_entry.get())
                user = self.user_entry.get()
                password = self.pass_entry.get()
                ...
                self.connected = True
                self.connect_btn.config(text="断开连接")
                self.status_var.set(f"状态: 已重新连接 ({host}:{port})")
                self.output("重连成功!")
                return True
            except Exception as e:
                self.output(f"重连失败: {str(e)}")
                self.status_var.set(f"状态: 重连失败 ({attempt}/{self.max_reconnect_attempts})")
                time.sleep(self.reconnect_delay)

        self.output(f"重连失败，已达到最大尝试次数 ({self.max_reconnect_attempts})")
    ...
    def send_at_commands_periodically(self):
        ...
        time.sleep(2)
        self.output("检测信号是否正常\r")
        self.channel.send('at+cpin?\r')  # 检测信号是否正常
        time.sleep(3)

        while count < max_count and not self.stop_event.is_set():
            try:
                # 检查连接状态
                if not self.connected or self.ssh_client is None or not self.ssh_client.get_transport().is_active():
                    self.output("连接已断开，尝试重新连接...")
                    if not self.handle_connection_loss():
                        self.output("无法恢复连接，停止发送AT命令")
                        break
                    ...
                    try:
                        ...
                        if transport and transport.is_active():
                            self.channel = transport.open_session()
                            self.channel.get_pty()
                            self.channel.exec_command("microcom /dev/ttyUSB13")
                            self.microcom_active = True
                            self.output("已重新建立microcom会话")
                    except Exception as e:
                        self.output(f"重新建立microcom会话失败: {str(e)}")
                        break
                ...
                if "Socket is closed" in str(e) or "Channel closed" in str(e):
                    self.output("检测到连接中断，尝试恢复...")
                    reconnect_count += 1
                    if reconnect_count > self.max_reconnect_attempts:  # 防止无限重连
                        self.output("重连尝试次数过多，停止发送AT命令")
                        self.output(f"已获得 {len(self.signal_strengths)} 条数据")
                        break
    ...
    def export_results(self):
        ...
        data_rows = [
            ...
            ["电池电压 (mV)", int(self.lithium_batt_volt) / 100 if self.lithium_batt_volt is not None else "N/A",
             "从日志中提取的lithium_Batt_volt值"],
            ["信号强度异常数量", self.signal_abnormal_count, "信号强度小于-95的数量"],
            ["信号强度异常占比(%)",
             int(self.signal_abnormal_count) / len(self.signal_strengths) * 100 if self.signal_strengths else "N/A",
             "信号强度小于-95的数据数量占总样本数的比例"],
        ...
        if self.signal_strengths:
            ...
            for i, strength in enumerate(self.signal_strengths):
                # 计算时间偏移（每10秒一个点）
                # timestamp = start_time - timedelta(seconds=(len(self.signal_strengths) - i) * 10)
                # time_str = timestamp.strftime("%H:%M:%S")
                ws.append([self.signal_time[i], strength, "是" if strength < -95 else "否"])
