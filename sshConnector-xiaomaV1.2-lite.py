...
class SSHClientApp:
    def __init__(self, root):
        self.root = root
        self.root.title("SSH 客户端 小马版v1.2-lite")
    ...
    def set_default_values(self):
        ...
        default_cmds = [
            "cd /mnt/sda2/video/" + current_year + "/" + current_month + "/" + current_date + " && ls -lr | grep total",
    ...
    def parse_output(self, cmd, output):
        ...
        # 解析视频存储状态
        if cmd == "cd /mnt/sda2/video/2025/202507/20250703 && ls -lr | grep total":
    ...
    def export_results(self):
        ...
        if self.signal_strengths:
            ...
            for i, strength in enumerate(self.signal_strengths):
                # 模拟时间戳（每5秒一次）
                minutes = i // 12  # 每5秒一次，12次为一分钟
                seconds = (i % 12) * 5
                time_str = f"18:{minutes:02d}:{seconds:02d}"
